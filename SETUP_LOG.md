...
| Step 1 워크스페이스 | ✅ |
| Step 2 conda env 2개 | ✅ |
| Step 3 스모크 3종 | ✅ 1·2 / ⏸ 3(데이터로더, 데이터 승인 대기) |
| Step 4 체크포인트 2종 | ✅ |
| Step 5 기준선 재현 | ✅ GR00T 61.7 (동봉 로그 63.3±3 안, 문서 65.3±3 에 0.6%p 미달) / ⚠️ OFT 19.8 (공개본 결함, starVLA#424) |
| Step 6 CI | ✅ |
| Step 7 Phase 2 패키지 | ✅ config 2종 + 런처 / ⏸ 데이터 다운로드(승인 게이트) |

사람이 결정할 사항: (1) 데이터 로컬 수령 승인 (8.8 GB, 디스크 충분) → `bash scripts/07_prepare_datasets.sh all`; (2) OFT 공개본 재업로드 대기 vs Phase 2 재학습으로 대체; (3) 클라우드 인스턴스(8×A100 80GB vs 8×H100), W&B entity, HF_TOKEN.
